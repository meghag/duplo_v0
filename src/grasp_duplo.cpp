...
namespace sort_duplos{
	DuploGrasper::DuploGrasper (ros::NodeHandle & n): n_(n)
	{
		...
		get_planning_scene_client_ = n_.serviceClient<arm_navigation_msgs::SetPlanningSceneDiff>(SET_PLANNING_SCENE_DIFF_NAME);
	...
	bool DuploGrasper::PickupObject()
	{
		...
		targetObject_.header.frame_id = "base_link";

		if (sensor_msgs::convertPointCloud2ToPointCloud(targetObject_,goal_pcd) == true)
		{
			...
			graspable_object.cluster = goal_pcd;
			graspable_object.reference_frame_id = "base_link";
			pickupGoal_.target = graspable_object;
		...
		///////////////////////// Newly Added - Oct 29 '12////////////////////////////
		arm_navigation_msgs::SetPlanningSceneDiff planning_scene_srv;
		geometry_msgs::PoseStamped pose_stamped;
		geometry_msgs::Vector3 dimensions;
		getClusterBoundingBox(targetObject_, pose_stamped, dimensions);
		arm_navigation_msgs::CollisionObject co;
		co.id = getNextObjectName();
		co.operation.operation = 0; //0: add, 1: remove, 2: detach and add; 3: attach and remove
		//arm_navigation_msgs::CollisionObjectOperation::ADD;
		co.header.frame_id = "base_link";
		co.header.stamp = ros::Time::now();
		arm_navigation_msgs::Shape object;
		object.type = 1;  //0: sphere; 1: box; 2: cylinder; 3: mesh
		//arm_navigation_msgs::Shape::CYLINDER;
		object.dimensions.resize(3);
		object.dimensions[0] = dimensions.x + 0.06;
		object.dimensions[1] = dimensions.y + 0.06;
		object.dimensions[2] = dimensions.z + 0.06;
		co.shapes.push_back(object);
		co.poses.push_back(pose_stamped.pose);
		...
		{
			ROS_INFO("Added collision object & operation");
			ROS_INFO("Number of collision objects = %zu", 
			         planning_scene_srv.response.planning_scene.collision_objects.size());
		//	return true;
		}
		////////////// End of newly added - Oct 29 2012 ////////////////
