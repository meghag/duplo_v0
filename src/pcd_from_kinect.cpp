...
namespace sort_duplos {
	KinectDataReader::KinectDataReader (ros::NodeHandle & n): n_(n)
	{
		...
		processDataClient_ = n_.serviceClient<duplo_v0::Process_PCD>("process_pcd");
		...
		newDataWanted_ = true;
		kinectDataSub_.subscribe(n_,kinectDataTopic_,1);
		tfFilter_ = new tf::MessageFilter<sensor_msgs::PointCloud2>(kinectDataSub_, listener_, "base_link", 1);
		tfFilter_->registerCallback( boost::bind(&KinectDataReader::kinectDataCallback, this, _1) );
	}
	...
	void KinectDataReader::kinectDataCallback(const sensor_msgs::PointCloud2::ConstPtr& input)             
	{
		input_ = *input;
		if (newDataWanted_) {
	...
	bool KinectDataReader::getNewDataCallback(duplo_v0::Get_New_PCD::Request &req,
